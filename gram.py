...
def p_operandlist1(p):
	"operandlist : operand"
	p[0] = [p[1]]

def p_operandlist2(p):
...
def p_instructionlist1(p):
	"instructionlist : empty"
	p[0] = []

def p_instructionlist2(p):
	"instructionlist : instructionlist instruction"
	p[0] = p[1]+[p[2]]

def p_return_operand1(p):
	"return_operand : '<' operand '>'"
	p[0] = p[2]

def p_return_operand2(p):
	"return_operand : empty"
	p[0] = 0
...
def p_rules2(p):
	...
	rule(p[2], p[4],p[10], p[11],p[6],p[8])
