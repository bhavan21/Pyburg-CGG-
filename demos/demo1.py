...
rules = {
	...
	3 : Rule("reg: PLUS(reg,CONST)", 1, "PLUS(reg,CONST)", [reg_NT, ] , 3, 1, instructions[2:3], 0),
	4 : Rule("reg: CONST", 1, "CONST", [] , 4, 1, instructions[3:4], 0),
}
	...
	def match(self):
		if self.value is None: assert 0, "No value in node\n"
			...
			if (	# reg: CONST
				...
				cost = 1;
				if (cost + 0 < self.cost[reg_NT]): # reg: CONST
					self.cost[reg_NT] = cost + 0;
					self.rule[reg_NT] = 4;
...
def setEnvironment(registers, variableOffsetFunc):
	...
	import RegAllocator
	global regAllocator
	regAllocator = RegAllocator.RegAllocator(regList)
