...
rules = {
	...
	2 : Rule("reg: PLUS(NAME,NAME)", 1, "PLUS(NAME,NAME)", [] , 2, 3, instructions[1:4], 0),
	3 : Rule("reg: PLUS(NAME,CONST)", 1, "PLUS(NAME,CONST)", [] , 3, 2, instructions[4:6], 0),
	4 : Rule("reg: PLUS(reg,CONST)", 1, "PLUS(reg,CONST)", [reg_NT, ] , 4, 1, instructions[6:7], 0),
}
...
class Node:
	...
	def match(self):
		if self.value is None: assert 0, "No value in node\n"
			...
			if (	# reg: PLUS(reg,CONST)
					self.children[1].value == 4  # CONST
				):

				cost = self.children[0].cost[reg_NT] + 1;
				if (cost + 0 < self.cost[reg_NT]): # reg: PLUS(reg,CONST)
					self.cost[reg_NT] = cost + 0;
					self.rule[reg_NT] = 4;

			if (	# reg: PLUS(NAME,CONST)
					self.children[0].value == 3 and # NAME
					self.children[1].value == 4  # CONST
				):

				cost = 2;
				if (cost + 0 < self.cost[reg_NT]): # reg: PLUS(NAME,CONST)
					self.cost[reg_NT] = cost + 0;
					self.rule[reg_NT] = 3;

			if (	# reg: PLUS(NAME,NAME)
					self.children[0].value == 3 and # NAME
					self.children[1].value == 3  # NAME
				):

				cost = 3;
				if (cost + 0 < self.cost[reg_NT]): # reg: PLUS(NAME,NAME)
					self.cost[reg_NT] = cost + 0;
					self.rule[reg_NT] = 2;
...
def getmatchedkids(p, rule):
	...
	if ruleno is None: assert 0, "No rulenumber associated with rule\n"
			...
			ruleno == 2 or # reg: PLUS(NAME,NAME)
			ruleno == 3 # reg: PLUS(NAME,CONST)
		):

		pass

	elif(
			ruleno == 4 # reg: PLUS(reg,CONST)
		):
...
# demonstating need for temporary registers
# a = b+c+5
