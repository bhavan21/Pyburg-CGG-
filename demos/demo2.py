...
rules = {
	...
	3 : Rule("reg: PLUS(reg,con)", 1, "PLUS(reg,con)", [reg_NT, con_NT, ] , 3, 1, instructions[2:3], 0),
	4 : Rule("reg: CONST", 1, "CONST", [] , 4, 1, instructions[3:4], 0),
	5 : Rule("con: CONST", 2, "CONST", [] , 5, 0,[], 1),
}
	...
	def match(self):
		if self.value is None: assert 0, "No value in node\n"
			...
			if (	# reg: PLUS(reg,con)
				...
				cost = self.children[0].cost[reg_NT] + self.children[1].cost[con_NT] + 1;
				if (cost + 0 < self.cost[reg_NT]): # reg: PLUS(reg,con)
...
def getmatchedkids(p, rule):
	...
	if ruleno is None: assert 0, "No rulenumber associated with rule\n"
		...
		pass
			...
			ruleno == 2 or # reg: PLUS(reg,reg)
			ruleno == 3 # reg: PLUS(reg,con)
		):
